...
export default function TenantProfile() {
  ...
  useEffect(() => {
    const fetchTenantProfile = async () => {
      try {
        ...
        const response = await axios.get(API_ENDPOINTS.UPDATE_TENANT_PROFILE, {
        ...
        if (response.data) {
          setAnswers({
            hasBeenEvicted: response.data.hasBeenEvicted || '',
            canPayMoreThanOneMonth: response.data.canPayMoreThanOneMonth || '',
            monthsAheadCanPay: response.data.monthsAheadCanPay || ''
          });

          // Update previews and documents with URLs from server
          const newPreviews = {
            proofOfIdentity: [],
            proofOfIncome: [],
            creditHistory: [],
          ...
          documentFields.forEach(field => {
            if (Array.isArray(response.data[field])) {
              response.data[field].forEach(doc => {
                if (doc?.url) {
                  newPreviews[field].push(doc.url);
                  newDocuments[field].push(new File(
  ...
  const { getRootProps: getIdentityProps, getInputProps: getIdentityInputProps } = useDropzone({
    onDrop: (files) => onDrop(files, 'proofOfIdentity'),
    accept: {
      'application/pdf': ['.pdf'],
      'image/*': ['.jpg', '.jpeg', '.png']
    },
  ...
  const { getRootProps: getIncomeProps, getInputProps: getIncomeInputProps } = useDropzone({
    onDrop: (files) => onDrop(files, 'proofOfIncome'),
    accept: {
      'application/pdf': ['.pdf'],
      'image/*': ['.jpg', '.jpeg', '.png']
    },
  ...
  const { getRootProps: getCreditProps, getInputProps: getCreditInputProps } = useDropzone({
    onDrop: (files) => onDrop(files, 'creditHistory'),
    accept: {
      'application/pdf': ['.pdf'],
      'image/*': ['.jpg', '.jpeg', '.png']
    },
  ...
  const { getRootProps: getRentalProps, getInputProps: getRentalInputProps } = useDropzone({
    onDrop: (files) => onDrop(files, 'rentalHistory'),
    accept: {
      'application/pdf': ['.pdf'],
      'image/*': ['.jpg', '.jpeg', '.png']
    },
  ...
  const { getRootProps: getAdditionalProps, getInputProps: getAdditionalInputProps } = useDropzone({
    onDrop: (files) => onDrop(files, 'additionalDocuments'),
    accept: {
      'application/pdf': ['.pdf'],
      'image/*': ['.jpg', '.jpeg', '.png']
    },
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Append each document if it exists
            ...
            <div key={index} className="relative">
              {p === 'pdf' || p.includes('.pdf') ? (
                <div className="flex items-center space-x-2 bg-gray-100 p-2 rounded-md">
                  ...
                  <span className="text-sm text-gray-600">{files[index]?.name || 'PDF Document'}</span>
                </div>
              ) : (
                <div className="relative">
                  <img
                    src={p.startsWith('data:') ? p : `${import.meta.env.VITE_API_URL}${p}`}
                    alt={`${label} preview`}
